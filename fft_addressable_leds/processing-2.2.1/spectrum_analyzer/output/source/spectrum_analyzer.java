...
public class spectrum_analyzer extends PApplet {
...
public void draw()
{
  ...
  for(int i = 0; i < 64; i++){    
    if (freq_height[i] > 255) {
      sig[i] = PApplet.parseByte(255);
    } else if (freq_height[i] < 0) {
      sig[i] = PApplet.parseByte(0);
    } else {
      sig[i] = PApplet.parseByte(PApplet.parseInt(freq_height[i]));
    }
  }
...
public void stop()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "spectrum_analyzer" };
    if (passedArgs != null) {
